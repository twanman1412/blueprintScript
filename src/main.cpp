...
int main (int argc, char *argv[]) {
    ...
    for (auto &f : hostFeatures) {
        featuresStr += (f.second ? "+" : "-") + f.first().str() + ",";
    }

    std::string error;
    const llvm::Target *target = llvm::TargetRegistry::lookupTarget(triple, error);
    if (!target) {
        logger.errorln("Target lookup failed: " + error);
        return 1;
    }

    llvm::TargetOptions opt;
    llvm::TargetMachine *targetMachine = target->createTargetMachine(
        triple, cpu, featuresStr, opt, llvm::Reloc::PIC_
    );
    ...
    std::string clangCommand = "clang -O3 \"" + llvmPath + "\" -o \"" + outputBase + "\"";
